...
namespace Alloy {
    ...
    public static class AlloyPackerCompositor {
        public static void CompositeMips(Texture2D target, AlloyCustomImportObject source,
            ...
            Profiler.BeginSample("Composite mips");
                        ...
                        for (int y = 0; y < h; ++y) {
                            ...
                            if (doNormal) {
                                ...
                                if (normalCache.NativeSize && mipLevel == 0) {
                                    normal = (Vector4)normalCache.Values[x + y * w];
                                }
                                else {
                                ...
                                var variance = Mathf.Clamp01(1.0f / (2.0f * kappa) - source.VarianceBias);

                                if (channel.OutputVariance) {
                                    input = variance;
                                } 
                                else if (channel.RoughnessCorrect) {
                            ...
                            switch (storeIndex) {
                                case 0: colors[y * w + x].r = input; break;
                                case 1: colors[y * w + x].g = input; break;
                                case 2: colors[y * w + x].b = input; break;
                                case 3: colors[y * w + x].a = input; break;
                            }
                        }
                    }
