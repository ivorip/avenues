...
public class AlloyDeferredRendererPlus : MonoBehaviour {
    ...
    // Shaders
    [HideInInspector] public Shader DeferredTransmissionBlit;
    [HideInInspector] public Shader DeferredBlurredNormals;
    ...
    private void OnDisable() {
        RemoveCommandBuffersFromCamera();
    }
    ...
    private void InitializeBuffers() {
        ...
        if ((m_isTransmissionEnabled || m_isScatteringEnabled)
            ...
            if (!m_isScatteringEnabled) {
                ...
                m_copyTransmission.GetTemporaryRT(outputRT, -1, -1, 0, FilterMode.Point, RenderTextureFormat.ARGB32);
                ...
                // Bind emission buffer for copy in following pass pass.
                ...
                m_deferredBlurredNormalsMaterial = new Material(DeferredBlurredNormals);
                ...
                m_renderBlurredNormals.GetTemporaryRT(outputRT, -1, -1, 0, FilterMode.Point, RenderTextureFormat.ARGB32);
                m_renderBlurredNormals.GetTemporaryRT(pingRT, halfWidth, halfHeight, 0, FilterMode.Point, RenderTextureFormat.ARGBHalf);
                m_renderBlurredNormals.GetTemporaryRT(pongRT, halfWidth, halfHeight, 0, FilterMode.Point, RenderTextureFormat.ARGBHalf);
        ...
        if (m_copyTransmission != null && !CameraHasCommandBuffer(c_copyTransmissionEvent, c_copyTransmissionBufferName)) {
            m_camera.AddCommandBuffer(c_copyTransmissionEvent, m_copyTransmission);
        }

        if (m_renderBlurredNormals != null && !CameraHasCommandBuffer(c_blurNormalsEvent, c_blurNormalsBufferName)) {
        ...
        if (m_isTransmissionEnabled || m_isScatteringEnabled) {
            ...
            if (m_isScatteringEnabled) {
                ...
                Shader.SetGlobalVector("_DeferredSkinParams",
                    new Vector3(SkinSettings.Weight, 1.0f / SkinSettings.MaskCutoff, SkinSettings.BumpBlur));
                Shader.SetGlobalVector("_DeferredSkinColorBleedAoWeights", SkinSettings.AoColorBleed);
                Shader.SetGlobalVector("_DeferredSkinTransmissionAbsorption", SkinSettings.Absorption);
            }
        }
    }
    ...
    private bool CameraHasCommandBuffer(CameraEvent evt, string name) {
        foreach (var buf in m_camera.GetCommandBuffers(evt)) {
            if (buf.name == name) {
                return true;
            }
        }
    ...
    private void RemoveCommandBuffersFromCamera() {
